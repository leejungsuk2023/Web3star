-- [DEPRECATED — 클라이언트 미사용] 이전에 “한 계정·한 기기” 강제 로그아웃용으로 추가했던 스키마입니다.
-- 현재 앱은 이 토큰/RPC를 호출하지 않습니다. 새 프로젝트에는 실행하지 않아도 됩니다.
-- 이미 적용된 DB는 컬럼·함수·publication 이 남아 있어도 동작에 문제 없습니다(정리만 원하면 수동 드롭).
--
